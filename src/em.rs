...
pub fn em_par(em_info: &EMInfo, nthreads: usize) -> EMResult {
    ...
    let eq_iterates: Vec<EqIterateT> = em_info.eq_map.iter().collect();
    let curr_counts: Vec<AtomicF64> = (0..em_info.txp_info.len())
    ...
    let mut fixed_point = |src: &[f64], dst: &mut [f64]| {
        ...
        for (out, value) in dst.iter_mut().zip(&curr_counts) {
            *out = value.load(Ordering::Relaxed);
        }
    };
