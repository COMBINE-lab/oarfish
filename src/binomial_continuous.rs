...
pub fn binomial_continuous_prob(txps: &mut Vec<TranscriptInfo>, rate: &str, bins: &u32, threads: usize) -> (Vec<usize>, Vec<usize>) {
    ...
    txps.par_iter_mut().enumerate().for_each(|(i, t)| {
        ...
        if *bins != 0 {
            ...
            match rate {
                "shr" | "shr_tlen" => {
                    panic!("Shared Rate cannot be used for binomial model");
                }
                "dr" => {
