...
fn perform_inference_and_write_output(
    ...
    if args.end_profile {
        // The end-profile term multiplies into the per-read coverage simplex;
        // seed it uniform when no coverage model filled it.
        if args.coverage_model == crate::prog_opts::CoverageModel::None {
            for i in 0..store.len() {
                let (s, e) = (store.boundaries[i], store.boundaries[i + 1]);
        ...
        let diagnostics =
            crate::util::polya_probability::apply_end_profile_probabilities(store, txps);
        info!(
            ?diagnostics,
            "applied per-transcript two-sided end-profile likelihood (dRNA)"
        );
        coverage_diagnostics["end_profile"] = serde_json::to_value(diagnostics)?;
    }
    ...
    if store.junc_informative_reads > 0 {
        info!(
            ...
            store
                .junc_informative_reads
                .to_formatted_string(&Locale::en),
            store
                .junc_all_mismatch_reads
                .to_formatted_string(&Locale::en),
    ...
    let (mut store, name_vec, aln_time) = std::thread::scope(
        |s| -> anyhow::Result<(
            ...
            info!(
                ...
                if n_pr > 0 {
                    100.0 * n_ji as f64 / n_pr as f64
                } else {
                    0.0
                },
                n_jm.to_formatted_string(&Locale::en),
                if n_ji > 0 {
                    100.0 * n_jm as f64 / n_ji as f64
                } else {
                    0.0
                },
            );
