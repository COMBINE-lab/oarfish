...
def main():
    ...
    merged = {}
    for k, v in d.items():
        o = pd.read_csv(f"{data_dir}/SIRV/quants/oarfish/{k}_quant.tsv", sep="\t")
        n = pd.read_csv(f"{data_dir}/SIRV/quants/nanocount/{k}_quant.tsv", sep="\s+")
    ...
    print(f"nano_count % reads mapped: {nzs/nas}\noarfish % reads mapped: {ozs/oas}")
